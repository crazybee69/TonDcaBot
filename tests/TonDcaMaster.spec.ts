...
describe('TonDcaMaster', () => {
    ...
    it('update create wallet fee and claim fees', async () => {
        await tonDcaMaster.send(deployer.getSender(),
            {
                value: toNano('0.01')
            },
            {
                $$type: 'UpdateCreateWalletFee',
                queryId: 0n,
                fee: toNano('10'),
            })

        const createWalletFee =  await tonDcaMaster.getGetCreateWalletFee()

        expect(createWalletFee).toEqual(toNano('10'))
        ...
        const balanceBeforeClaim = await deployer.getBalance()
        ...
        const balanceAfterClaim = await deployer.getBalance()
